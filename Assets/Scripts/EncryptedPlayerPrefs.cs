...
public class EncryptedPlayerPrefs  {
    ...
    private static string privateKey
    {
        get
        {
            ...
            {
                if(!PlayerPrefs.HasKey("ProperPk"))
                {
                    string newKey = Md5(System.DateTime.Now.Ticks.ToString());
                    PlayerPrefs.SetString("ProperPk", newKey);
                }
                m_pKey = PlayerPrefs.GetString("ProperPk");
                return m_pKey;
            }
        }
        set
    ...
    public static void SaveEncryption(string key, string type, string value) {
        string check = Md5(type + "_" + privateKey + "_" + value);
        PlayerPrefs.SetString(key + "chk", check);
    }

    public static bool CheckEncryption(string key, string type, string value) {
        string check = Md5(type + "_" + privateKey + "_" + value);
        if(!PlayerPrefs.HasKey(key + "chk")) return false;
        string storedCheck = PlayerPrefs.GetString(key + "chk");
    ...
    public static void DeleteKey(string key) {
        ...
        PlayerPrefs.DeleteKey(key + "_chk");
    }
}